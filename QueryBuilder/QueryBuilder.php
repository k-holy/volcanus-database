...
class QueryBuilder
{
	...
	/**
	 * @var Volcanus\Database\QueryBuilder\QueryBuilderInterface
	 */
	private $builder;
	...
	public function parameters($tableName, $columns)
	{
		...
		foreach ($columns as $columnName => $value) {
			$noConvert = false;
			if (0 === strncmp($columnName, self::PREFIX_NO_CONVERT, 1)) {
...
SQL;
	...
	public function whereExpressions($tableName, $tableAlias = null, $columns = array())
	{
		...
		foreach ($columns as $key => $value) {
			...
			$negative = false;
			$noConvert = false;
			if (0 === strncmp($columnName, self::PREFIX_NEGATIVE, 1)) {
				$columnName = substr($columnName, 1);
				$negative = true;
			} elseif (0 === strncmp($columnName, self::PREFIX_NO_CONVERT, 1)) {
				$columnName = substr($columnName, 1);
				$noConvert = true;
			}
			if (!array_key_exists($columnName, $metaColumns)) {
